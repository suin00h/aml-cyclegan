...
class cityscapes:
    def __init__(self, data_path):
        ...
        self.rgb2trainId = {label.color: label.trainId for label in labels.labels if label.trainId != 255}
    ...
    def rgb_to_trainId(self, image_rgb, tolerance=1):
        h, w, _ = image_rgb.shape
        trainId_image = np.full((h, w), 255, dtype=np.uint8)  # default ignore

        for color, trainId in self.rgb2trainId.items():
            color_arr = np.array(color).reshape(1, 1, 3)
            diff = np.abs(image_rgb - color_arr)
            mask = np.all(diff <= tolerance, axis=-1)
            trainId_image[mask] = trainId
